...
def verifySaveDirectory(group,x=0,y=0): 
        ...
        if 'GameDatabase' in dir:
                filename = dir.replace('GameDatabase','Decks').replace('ffd507ab-7cfc-47b8-92c8-1e85b5a26d7f','Illuminati')
        else:
                filename = "Decks\Illuminati".join(dir.rsplit('OCTGN',1))
        #notify(filename)
        try:
                nt.mkdir(filename)
        except:
...
def savePiles(name, sections, piles, skipInvestigator, isShared):
	...
	with open(filename, 'w+') as f:
		...
		f.write('<deck game="ffd507ab-7cfc-47b8-92c8-1e85b5a26d7f">\n')
		for s in sections:
			if len(sections[s]) > 0:
